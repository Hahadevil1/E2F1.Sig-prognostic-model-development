...
rm (list=ls())
gc()
setwd("~/if01/Analysis/TMP_Analysis/GM")
...
load('data/ios/ios.Rdata');names(ios) #10 ICT RNA-Seq cohorts
load("~/if01/Analysis/Analysis_JLX/Project07_scRNA&Bulk&ICI&PanCancer/Step3_Circos_plot/Project01_Unfolded_Protein/D04_E2F1.Sig.RData")
load("~/if01/Analysis/Analysis_JLX/Project07_scRNA&Bulk&ICI&PanCancer/Step6_Model_ICB/Project01_Unfolded_Protein/Chr02_GetICB_Genes/D07_Final_HubGenes_ICB.RData")
...
###########################################################################################################
####===============================3. Model Training and Validation with E2F1.Sig======================####
###########################################################################################################
...
save(training,file='./D01_training_set.Rdata')
save(validation,file='./D02_validation_set.Rdata')
save(test,file='./D03_test_set.Rdata')
...
CompareModel <- function(training, validation, method,sig){
  ...
  auc <- lapply(ls_model,function(model.tune){
    if(class(model.tune)[1] == 'predictor'){
...
stopCluster(cl)
res$auc$ROC <- ifelse(res$auc$ROC < 0.5,c(1-res$auc$ROC),res$auc$ROC)
...
###########################################################################################################
####===============================4. Model Testing with E2F1.Sig======================================####
###########################################################################################################


sig=E2F1.Sig
AUC_E2F1.Sig <- lapply(c('validation', 'testing',grp[c(3,7,8,9,10)],'SKCM'), function(g){
  if(g == 'validation') {
    new = validation
  }else if (g == 'testing'){
    new <- test
  } else if(g == 'SKCM') { #SKCM cohort: Hugo 2016 + Van Allen 2015
    new = test[test$batch %in% grp[c(3,7)], colnames(test) %in% c('response',sig)]
  } else{
    new <- test[test$batch == g, colnames(test) %in% c('response',sig)]
  }
  
  prob <- predict(res[['model']][[which.max(res$auc$ROC)]],new[,-1],type = "prob") # use 'nb' model
  pre <- predict(res[['model']][[which.max(res$auc$ROC)]],new[,-1]) # use 'nb' model
  test_set <- data.frame(obs = new$response, NR = prob[,'NR'], R = prob[,'R'], pred=pre)
...
}) %>% do.call(rbind,.) %>% `rownames<-`(c('validation', 'testing',grp[c(3,7,8,9,10)],'SKCM'))  %>% `colnames<-`('AUC')

AUC_E2F1.Sig[which(AUC_E2F1.Sig < 0.5)] <- 1-AUC_E2F1.Sig[which(AUC_E2F1.Sig < 0.5)]
AUC_E2F1.Sig

###########################################################################################################
####==================5. Compare E2F1.Sig with other Signatures in testing datasets====================####
###########################################################################################################
...
getImmmunCells.Sig <- function(){
  new = test[test$batch %in% grp[c(3,7)], colnames(test) %in% c('response',sig)]#SKCM cohort: Hugo 2016 + Van Allen 2015
...
CRMA_combat <- CRMA_data[!CRMA_data$batch %in% grp[c(3,7,8,9,10)],]
...
getCRMA.Sig <- function(){
  new <- CRMA_data[CRMA_data$batch %in% grp[c(3,7)], colnames(CRMA_data) %in% c('response',sig)]#SKCM cohort: Hugo 2016 + Van Allen 2015
  new$response <- ifelse(new$response=='0','NR','R')%>% factor(.,levels = c('R','NR'))
  roc <- ROCit::rocit(score = compositions::geometricmeanRow(new[,2:ncol(new)]),
                      class = new$response,
                      negref = 'NR')
...
getIPRES.Sig <- function(){
  new <- data[data$batch %in% grp[c(3,7)], ] #SKCM cohort: Hugo 2016 + Van Allen 2015
...
#IMS scores = Average gene expression of 18 immunosE2F1ession genes
...
AUC[['pancancer']] <- list(E2F1.Sig = data.frame(AUC = AUC_E2F1.Sig[-8,]),
...
AUC[['SKCM']] <- list(E2F1.Sig = AUC_E2F1.Sig[8,],          
...
setwd("~/if01/Analysis/Analysis_JLX/Project07_scRNA&Bulk&ICI&PanCancer/Step6_Model_ICB/Project01_Unfolded_Protein/Chr04_ComparedROC")

save(AUC,file='./D04_AUC.Rdata')
